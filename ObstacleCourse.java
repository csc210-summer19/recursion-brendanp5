...
public class ObstacleCourse {
	...
	private boolean findExit(int row, int col) {
		...
		else if (course[row][col] == TRIED)
			return false;

		if (course[row][col] == '+')
			return false;

		if (escapeCheck(row, col)) {
			escaped = true;
			foundRow = row;
			foundCol = col;
		}
			...
			else if (findExit(row, col + 1))
				escaped = true;

			else if (findExit(row - 1, col))
				escaped = true;
	...
	private boolean escapeCheck(int row, int col) {
		if (row == 0 || col == 0)
			return true;
		if (row == course.length - 1 || col == course[0].length - 1)
			return true;
		return false;
	}
